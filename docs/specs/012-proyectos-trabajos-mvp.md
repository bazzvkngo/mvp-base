...
La fórmula coherente es `costoTotal = materiales de Ventas confirmadas + materiales adicionales netos + HH vigentes + gastos directos vigentes + gastos indirectos vigentes`, `resultado = valorComercial - costoTotal` y `rentabilidadPct = resultado / valorComercial × 100`. Los materiales de Venta provienen exclusivamente de sus efectos físicos de inventario y del costo histórico congelado al descontar stock; servicios y actividades permanecen sólo como ingreso. Vincular la Venta no crea otra salida. Los consumos adicionales conservan `SALIDA_PROYECTO`/`DEVOLUCION_PROYECTO` y su costo congelado. Cuando existe cualquiera de estas fuentes de Inventario, los gastos vigentes con categoría `MATERIAL` quedan informados pero excluidos del costo para evitar doble imputación. Sin libro de materiales se conservan como costo directo legacy.

Una Venta cancelada deja de aportar ingreso y costo de materiales al balance; su reversa comercial restituye el stock según el contrato de Ventas. Los consumos adicionales del Proyecto no se alteran. Un efecto legacy sin costo congelado se informa como no disponible y no se recalcula con el costo vigente.

...
export default function ReceptionDocumentImportDialog({
  ...
  const analyze = async () => {
    ...
    try {
      ...
      setError(analysisError.message || "No se pudo analizar el documento.");
    } finally {
      setAnalyzing(false);
    }
  };
    ...
    <div className="reception-import">
      ...
      {rows.length > 0 && <>
        <section className="reception-import__document">
          <div><span>Proveedor</span><strong>{providerSnapshot?.razonSocial || "Proveedor de la OC"}</strong></div>
        ...
        <div className="erp-table-region">
          <table className="erp-table reception-import__table">
            <thead><tr><th>Ítem documento</th><th>Ítem OC</th><th>Cant.</th><th>Costo</th><th>Desc. %</th><th>Estado</th></tr></thead>

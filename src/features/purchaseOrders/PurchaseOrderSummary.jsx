...
export default function PurchaseOrderSummary({disabled, onCancel, onEmit, onSave, saving, totals}) {
  return (
    <aside className="po-panel po-summary">
      ...
      {!disabled && (
        <div className="po-summary__actions">
          <button type="button" className="po-button po-button--primary" disabled={saving} onClick={onSave}>Guardar borrador</button>
          <button type="button" className="po-button po-button--issued" disabled={saving} onClick={onEmit}>Emitir orden</button>
          {onCancel && <div className="po-summary__danger"><button type="button" className="po-button po-button--danger" disabled={saving} onClick={onCancel}>Cancelar orden</button></div>}
        </div>
      )}
    </aside>
  );

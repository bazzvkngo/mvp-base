    ...
    <article className="po-document-preview">
      <header className="po-document-preview__header">
        <div className="po-document-preview__company">
          ...
          <div><h2>{brand}</h2><p>{company.razonSocial !== brand ? company.razonSocial : ""}</p><p>{company.rut ? `RUT ${company.rut}` : ""}</p><p>{[company.direccion, company.comunaNombre || company.comuna, company.regionNombre || company.region].filter(Boolean).join(" · ")}</p><p>{[company.email, company.telefono].filter(Boolean).join(" · ")}</p></div>
        </div>
      ...
      <section className="po-document-preview__provider-grid">
        ...
        <div><h3>Entrega y condiciones</h3><p><strong>Entrega estimada:</strong> {order.fechaEntregaEstimada || "No informada"}</p><p><strong>Dirección:</strong> {order.direccionEntrega || "No informada"}</p><p><strong>Pago:</strong> {payment(order.condicionesPago || provider.condicionesPago) || "No informado"}</p><p><strong>Moneda:</strong> CLP</p></div>
      </section>
      ...
      <section className="po-document-preview__totals"><div><span>Subtotal</span><strong>{money(order.subtotal)}</strong></div><div><span>Descuentos</span><strong>{discountMoney(order.descuentoTotal)}</strong></div><div><span>Neto</span><strong>{money(order.neto)}</strong></div><div><span>IVA 19%</span><strong>{money(order.iva)}</strong></div><div className="po-document-preview__total"><span>Total</span><strong>{money(order.total)}</strong></div></section>

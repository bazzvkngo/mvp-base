  ...
  return (
    <article className="po-document-preview">
      ...
      <section className="po-document-preview__provider-grid">
        <div><h3>Proveedor</h3><strong>{provider.razonSocial || "Proveedor no seleccionado"}</strong><p>{provider.identificadorFiscalValor || provider.rut ? `${provider.identificadorFiscalTipo || "Identificación fiscal"} ${provider.identificadorFiscalValor || provider.rut}` : ""}</p><p>{provider.personaContacto ? `Contacto: ${provider.personaContacto}` : ""}</p><p>{[provider.email, provider.telefono].filter(Boolean).join(" · ")}</p><p>{[provider.direccion, provider.comunaNombre, provider.regionNombre].filter(Boolean).join(", ")}</p></div>

...
function OrderActions({canManage, onCancel, onContinue, onEdit, onOpen, onSend, order, processing, receptionStatus}) {
  return <div className="po-history__actions po-history__actions--purchase-orders">
    {order.estado === "borrador" && canManage
      ...
      : order.estado === "emitida" && canManage
        ? <button type="button" className="po-history__primary" disabled={processing} onClick={() => onContinue(order)}>{getSupplierResponseState(order) === "rechazada" ? "Actualizar confirmación" : receptionStatus === "recibida_total" ? "Ver recepciones" : "Registrar recepción"}</button>
        : <button type="button" className="po-history__primary" onClick={() => onOpen(order)}>Ver</button>}
    {canManage && order.estado !== "cancelada" && <button type="button" className="po-history__danger" disabled={processing} onClick={() => onCancel(order)}>Cancelar</button>}

...
import {cancelarOrdenCompra, createPurchaseOrderDuplicateRequestId, duplicarOrdenCompraComoBorrador, emitirOrdenCompra, listarOrdenesCompra} from "../services/purchaseOrderService";
...
import {buildPurchaseOrderPdfAttachment, downloadPurchaseOrderPdf, sharePurchaseOrderWhatsApp} from "../utils/purchaseOrderPdf";
...
function OrderActions({canManage, onCancel, onContinue, onDuplicate, onEdit, onOpen, onSend, order, processing, receptionStatus}) {
  return <div className="po-history__actions po-history__actions--purchase-orders">
    ...
    {order.estado === "borrador" && canManage && <button type="button" onClick={() => onEdit(order)}>Editar</button>}
...
export default function PurchaseOrdersPage({businessId, role}) {
  ...
  return <main className="erp-page po-history">
    ...
    <section className="erp-panel erp-history-panel"><div className="erp-panel-header"><div><h2 className="erp-panel-title">Órdenes registradas</h2><p className="erp-secondary-text">{filtered.length} órdenes</p></div></div>
      ...
      {loading ? <div className="erp-empty-state">Cargando órdenes...</div> : <><section className="erp-table-region po-history__desktop"><table className="erp-table po-history__table"><thead><tr><th>Orden</th><th>Proveedor</th><th>Total</th><th>Estado OC</th><th>Respuesta proveedor</th><th>Recepción</th><th>Acciones</th></tr></thead><tbody>{filtered.map((order) => { const receptionStatus = getOrderReceptionStatus(order, receptions); const progress = getOrderReceptionProgressDisplay(order, receptions); return <tr className={order.id === recentOrderId ? "po-history__recent" : ""} key={order.id}><td><button type="button" className="po-inline-link" onClick={() => open(order)}>{order.numero}</button></td><td><strong>{order.proveedorSnapshot?.razonSocial}</strong><small>{order.proveedorSnapshot?.rut}</small></td><td>{money(order.total)}</td><td><span className={`po-status po-status--${order.estado}`}>{labels[order.estado]}</span></td><td>{responseLabel(order)}</td><td><span className={`po-status po-status--${receptionStatus}`}>{getOrderReceptionStatusLabel(receptionStatus)}</span><small>{progress.received} / {progress.requested}</small></td><td><OrderActions canManage={canManage} onCancel={setPendingAction} onContinue={continueOrder} onDuplicate={duplicate} onEdit={openWorkspace} onOpen={open} onSend={setEmailOrder} order={order} processing={processingId === order.id} receptionStatus={receptionStatus} /></td></tr>; })}{!filtered.length && <tr><td colSpan="7" className="po-history__empty">No hay órdenes coincidentes.</td></tr>}</tbody></table></section><section className="po-history__cards" aria-label="Órdenes de compra">{filtered.map((order) => { const receptionStatus = getOrderReceptionStatus(order, receptions); const progress = getOrderReceptionProgressDisplay(order, receptions); return <article className={`po-history-card ${order.id === recentOrderId ? "po-history__recent" : ""}`} key={order.id}><header><button type="button" className="po-inline-link" onClick={() => open(order)}>{order.numero}</button><span className={`po-status po-status--${order.estado}`}>{labels[order.estado]}</span></header><div className="po-history-card__provider"><strong>{order.proveedorSnapshot?.razonSocial}</strong><span>{money(order.total)}</span></div><dl><div><dt>Respuesta</dt><dd>{responseLabel(order)}</dd></div><div><dt>Recepción</dt><dd>{getOrderReceptionStatusLabel(receptionStatus)} · {progress.received} / {progress.requested}</dd></div></dl><OrderActions canManage={canManage} onCancel={setPendingAction} onContinue={continueOrder} onDuplicate={duplicate} onEdit={openWorkspace} onOpen={open} onSend={setEmailOrder} order={order} processing={processingId === order.id} receptionStatus={receptionStatus} /></article>; })}</section></>}
    </section>
    {selectedOrder && <div className="print-only"><PurchaseOrderPrintView company={company} order={selectedOrder} /></div>}
    <ResponsiveDialog className="po-order-preview-dialog" layerClassName="po-order-preview-layer no-print" open={Boolean(selectedOrder)} onClose={() => setSelectedOrder(null)} eyebrow="Órdenes de compra" title={selectedOrder ? `Orden de compra ${selectedOrder.numero}` : "Vista previa"} description="Revisa el documento guardado y compártelo con el proveedor." size="large" footer={selectedOrder ? <div className="erp-actions po-order-preview-actions">{canManage && selectedOrder.estado !== "cancelada" && <Button type="button" disabled={processingId === selectedOrder.id} onClick={() => { setEmailOrder(selectedOrder); setSelectedOrder(null); }}>Correo</Button>}{canManage && selectedOrder.estado !== "cancelada" && <Button type="button" variant="secondary" disabled={processingId === selectedOrder.id} onClick={() => shareWhatsApp(selectedOrder)}>WhatsApp</Button>}<Button type="button" variant="secondary" disabled={processingId === selectedOrder.id} onClick={() => downloadPdf(selectedOrder)}>Descargar PDF</Button><Button type="button" variant="secondary" onClick={() => window.print()}>Imprimir</Button>{canManage && selectedOrder.estado === "borrador" && <Button type="button" variant="secondary" onClick={() => openWorkspace(selectedOrder)}>Editar</Button>}<Button type="button" variant="secondary" onClick={() => setSelectedOrder(null)}>Volver al listado</Button></div> : null}>

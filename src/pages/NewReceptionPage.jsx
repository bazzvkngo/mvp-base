...
export default function NewReceptionPage({businessId, role}) {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      sileo.success({title: "Recepcion confirmada", description: result.compra ? `${result.compra.numero} fue registrada automáticamente.` : "La recepción histórica conservó su compra original."});
      ...
      if (shouldReconcileReceptionConfirmation(error)) {
        try {
          ...
          if (authoritative?.estado === "confirmada" && authoritative?.stockAplicado) {
            ...
            if (authoritative.compraId) {
              setLinkedPurchase(await obtenerCompra(businessId, authoritative.compraId));
            }
            navigate(`/recepciones/${reception.id}`, {replace: true});
  ...
  const openPurchase = () => {
    ...
    navigate(`/compras/${reception.compraId}`);
  };
  const applyImport = ({items, documentoOrigen, aplicadas, omitidas}) => {
    ...
    sileo.success({
      ...
      description: `${aplicadas} línea${aplicadas === 1 ? "" : "s"} rellenada${aplicadas === 1 ? "" : "s"}${omitidas ? `; ${omitidas} quedaron sin asociar.` : "."} Aún no se modificó el stock.`,
    });
  };
  ...
  return <main className="po-workspace">
    ...
    {reception.estado === "confirmada" && <section className="reception-success" role="status" aria-live="polite">
      <div className="reception-success__copy">
        ...
        <p className="reception-success__next">{reception.compraId ? reception.compraEstado === "revertida" ? `La compra asociada fue revertida${reception.compraReversionMotivo ? `: ${reception.compraReversionMotivo}` : "."}` : "La compra quedó registrada automáticamente y está disponible para consulta." : "Esta recepción histórica conserva el vínculo económico previo de su orden."}</p>
      </div>
      {purchaseAction && <div className="reception-success__action">
        <Button type="button" variant="secondary" disabled={processing} onClick={openPurchase}>Ver compra</Button>

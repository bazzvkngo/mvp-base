...
function StatisticsPage({businessId, currencyCode = "CLP", role = ""}) {
  ...
  const canViewProfitability = ["OWNER", "ADMIN", "FINANZAS"].includes(String(role || "").toUpperCase());
  ...
  return (
    <section className="erp-page statistics-page reports-page">
      ...
      {!reportState.loading && !reportState.error && activeTab === "projects" && (
        <div className="report-section-stack">
          <div className="financial-data-note"><strong>Costos del expediente.</strong> Gastos y HH vigentes provienen del TRB; la rentabilidad usa la Function autoritativa y sólo es visible a OWNER/ADMIN/FINANZAS.</div>

...
export default function WorksPage({businessId, currencyCode, currentUserUid, role}) {
  ...
  const [detail, setDetail] = useState({tareas: [], notas: [], historial: [], vinculos: [], cotizaciones: [], ventas: [], gastos: [], horasHombre: [], materiales: []});
...
function MaterialsSection({businessId, canManage, currency, loading, movements, processing, products, role, runAction, workId}) {
  const [draft, setDraft] = useState({itemId: "", cantidad: "", fecha: chileToday()});
  const [returnDrafts, setReturnDrafts] = useState({});
  const canConsume = canManage || role === "MEMBER";
  const exits = movements.filter((movement) => movement.tipo === "SALIDA_PROYECTO");
  const returns = movements.filter((movement) => movement.tipo === "DEVOLUCION_PROYECTO");
  const valuedTotal = exits.reduce((sum, movement) => sum + movement.costoTotal, 0) - returns.reduce((sum, movement) => sum + movement.costoTotal, 0);

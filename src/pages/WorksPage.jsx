...
import {actualizarTrabajo, agregarNotaTrabajo, agregarTareaTrabajo, cambiarEstadoTareaTrabajo, cambiarEstadoTrabajo, cargarFichaTrabajo, createWorkRequestId, crearTrabajo, eliminarTareaTrabajo, listarTrabajos} from "../services/workService.js";
...
export default function WorksPage({businessId, role}) {
  ...
  const [detail, setDetail] = useState({tareas: [], notas: [], historial: []});
  ...
  const openDetail = (work) => { setSelectedWork(work); setTaskTitle(""); setNoteText(""); loadDetail(work.id); };
  ...
  const addTask = (event) => { event.preventDefault(); const value = taskTitle.trim(); if (!value) return; runDetailAction("task-new", () => agregarTareaTrabajo(businessId, selectedWork.id, value)).then((success) => {if (success) setTaskTitle("");}); };
